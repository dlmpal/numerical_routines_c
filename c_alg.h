...
void
matrix_mult_complex(int rows1, int cols1, int cols2, double complex matrix1[][cols1], double complex matrix2[][cols2],
    ...
    if (rows1 <= 0 || cols2 <= 0 || cols1 <= 0) {
        ...
        register int j;
        register int k;
        double complex local_sum = 0 + 0 * I;
        for (i = 0; i < rows1; i++) {
            for (j = 0; j < cols2; j++) {
                for (k = 0; k < cols1; k++) {
...
void create_dft_matrix(int dim, double complex dft_matrix[][dim]) {
    register int i;
    register int j;
    register double PI = acos(-1);
    register double complex current_freq = cexp(I * -2 * PI / dim);
    for (i = 0; i < dim; i++) {
        for (j = 0; j < dim; j++) {
